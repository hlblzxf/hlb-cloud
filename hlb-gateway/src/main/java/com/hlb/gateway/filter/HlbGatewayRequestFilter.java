...
@Slf4j
@Component
public class HlbGatewayRequestFilter implements GlobalFilter {
    @Autowired
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ServerHttpRequest build = request.mutate().header(HlbConstant.ZUUL_TOKEN_HEADER, new String(token)).build();
    ...
    private Mono<Void> checkForbidUri(ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (!shouldForward) {
            ...
            return makeResponse(response, hlbResponse);
        }
        return null;
    }

    private Mono<Void> makeResponse(ServerHttpResponse response, HlbResponse hlbResponse) {
        response.setStatusCode(HttpStatus.FORBIDDEN);
        response.getHeaders().add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
        DataBuffer dataBuffer = response.bufferFactory().wrap(JSONObject.toJSONString(hlbResponse).getBytes());
